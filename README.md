...
- 🔄 **API** — Совместим с Yandex SpeechKit STT API v2
- 🧠 **Инференс** — NVIDIA Triton + T-one CTC пайплайн
- 🔍 **VAD** — Silero VAD + KenLM для пост-обработки
- 🗄️ **Вектора** — Qdrant для хранения и поиска эмбеддингов
...
- 🎯 Извлекает **192-dim** эмбеддинги для верификации
- 📦 Полный цикл: подготовка данных → обучение → инференс API
